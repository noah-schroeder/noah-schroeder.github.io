...
def update_scholar_stats():
  try:
      ...
      for i, pub in enumerate(publications[:3]):
          ...
          # Extract venue (journal/conference) information
          venue = (filled_pub['bib'].get('journal') or 
                  filled_pub['bib'].get('venue') or 
                  filled_pub['bib'].get('conference') or 
          ...
          # Debug print
          print(f"Processing publication: {pub_data['title']}")
